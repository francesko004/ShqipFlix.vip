...
export function EpisodeSelector({ tvId, currentSeason, currentEpisode, totalSeasons, episodes }: EpisodeSelectorProps) {
    ...
    return (
        <div className="flex items-center gap-4 bg-white/5 backdrop-blur-sm p-4 rounded-lg border border-white/10">
            ...
            <div className="flex gap-2">
                ...
                <Link
                    ...
                    className={`px-3 py-2 bg-white/10 hover:bg-white/20 text-white rounded text-sm transition-colors ${currentEpisode === 1 ? "opacity-50 pointer-events-none" : ""
                        }`}
                >
                    ← Prev
                </Link>
                ...
                <div className="relative">
                    <button
                        ...
                        className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded text-sm flex items-center gap-2 min-w-[100px] justify-between transition-colors"
                    >
                    ...
                    {isSeasonOpen && (
                        <div className="absolute top-full mt-1 left-0 bg-[#1a1a2e] border border-white/10 rounded-lg shadow-xl max-h-64 overflow-y-auto z-50 min-w-[100px]">
                            {seasons.map((season) => (
                                <Link
                                    key={season}
                                    href={`/tv/${tvId}?season=${season}&episode=1`}
                                    onClick={() => setIsSeasonOpen(false)}
                                    className={`block px-4 py-2 text-sm hover:bg-white/10 transition-colors ${season === currentSeason ? "bg-red-600/20 text-red-500 font-bold" : "text-white"
                                        }`}
                                >
                                    Season {season}
                                </Link>
                ...
                <div className="relative">
                    <button
                        ...
                        className="px-4 py-2 bg-white/10 hover:bg-white/20 text-white rounded text-sm flex items-center gap-2 min-w-[140px] justify-between transition-colors"
                    >
                    ...
                    {isEpisodeOpen && (
                        <div className="absolute top-full mt-2 left-0 w-[280px] sm:w-[350px] bg-[#1a1a2e] border border-white/10 rounded-xl shadow-2xl max-h-[400px] overflow-y-auto z-50 p-2 space-y-1">
                            {episodes.map((episode) => (
                                <Link
                                    key={episode.id}
                                    href={`/tv/${tvId}?season=${currentSeason}&episode=${episode.episode_number}`}
                                    onClick={() => setIsEpisodeOpen(false)}
                                    className={`flex gap-3 p-2 rounded-lg transition-all border group ${episode.episode_number === currentEpisode
                                        ? "bg-red-600/20 border-red-600/50"
                                        : "hover:bg-white/5 border-transparent"
                                        }`}
                                >
                                    <div className="relative w-20 aspect-video rounded overflow-hidden flex-shrink-0 bg-black/40">
                                        {episode.still_path ? (
                                            <img
                ...
                {hasNextEpisode ? (
                    ...
                    <Link
                        ...
                        className="px-3 py-2 bg-red-600/20 hover:bg-red-600/40 text-red-500 rounded text-sm transition-colors font-bold"
                    >
                        Season {currentSeason + 1} →
                    </Link>
                ) : (
                    <button className="px-3 py-2 bg-white/10 text-white rounded text-sm opacity-50 cursor-not-allowed">
                        Next →
                    </button>
                )}
            </div>
